...
def currency_detection():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(filepath)
            currency_result = detect_currency(filepath)
            # Convert dict to string for display
            if isinstance(currency_result, dict):
                currency_str = ", ".join([f"{name}: {conf:.2f}" for name, conf in currency_result.items()])
            else:
                currency_str = str(currency_result)
            return jsonify({"currency": currency_str}), 200
